      ...
      if (StringUtils.isEmpty(templateId)) {
        try {
          ...
          if (msg != null && msg.toLowerCase().contains("duplicate")) {
            ...
            //TODO retrieving the mail via name does not work, API will throw error even when the URL is correct
            //int id = client.getEmailTemplateIdByName(marketingContext, newsletterContext, token, title);
            //createTemplateResult = client.updateEmailTemplate(marketingContext, newsletterContext, token, String.valueOf(id), title, body);
            //TODO workaround for not updating email templates
